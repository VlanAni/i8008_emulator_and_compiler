...
int main()
{
    ...
    const char* jump_instr[28] = {"JMP", "JNC", "JNZ", "JP", "JPO", "JC", "JZ", "JM", "JPE", "CALL", "CNC", "CNZ", "CP", "CPO", "CC", "CZ", "CM", "CPE", "RET", "RNC", "RNZ", "RP", "RPO", "RC", "RZ", "RM", "RPE", "RST"};
    ...
    for (int i = 0; i < 28; i++)
    {
        calculated_hash = PolinHash(jump_instr[i]);
        if (hashes[calculated_hash])
        {
            amount_of_intersects++;
            printf("%" PRIu32 "\n", calculated_hash);
